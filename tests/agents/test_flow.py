...
def _flow():
    return Flow(
        ...
        module="m",
        steps=[
            FlowStep(id="s1", say="bước 1", next=[FlowTransition(when="xong", goto="s2")]),
            FlowStep(id="s2", say="bước 2", next=[FlowTransition(when="ok", goto="done")]),
        ],
...
async def test_advances_active_flow_step():
    ...
    session = SessionState(conversation_id="cv1", active_flow_id="f1", current_step_id="s1")
    seq = [{"stop_reason": "end_turn", "text": "Làm bước 2 nhé [[goto:s2]]", "tool_calls": []}]
    with patch("agent_customer_support.agents.flow.complete_with_tools", side_effect=seq):
...
async def test_outcome_clears_session():
    fs = AsyncMock()
    fs.get.return_value = _flow()
    session = SessionState(conversation_id="cv1", active_flow_id="f1", current_step_id="s2")
...
def test_build_assistant_message_anthropic_echoes_tool_use():
    out = {
        "text": "checking",
        "tool_calls": [{"id": "t1", "name": "get_flow", "input": {"flow_id": "f1"}}],
    }

...
async def _build_core(llm_outputs):
    ...
    core.customers.get.return_value = CustomerProfile(
        customer_id="c1", name="C1", enabled_modules=["m"]
    )
    core.conversations = AsyncMock()
    core.sessions = AsyncMock()
    core.rag = AsyncMock()
    core.flow_store = AsyncMock()
    core.backlog = AsyncMock()
    core.escalator = AsyncMock()
    ...
    seq = [
        {
            ...
            "text": None,
            "tool_calls": [{"id": "t1", "name": "search_knowledge", "input": {"query": "x"}}],
            "raw": None,
        },
        {"stop_reason": "end", "text": "Đáp án dựa trên tài liệu", "tool_calls": [], "raw": None},
    ]
    with patch("agent_customer_support.agent.core.complete_with_tools", side_effect=seq):
        reply = await core.handle_turn(
            customer_id="c1", conversation_id="cv1", user_msg="cách làm X"
        )
